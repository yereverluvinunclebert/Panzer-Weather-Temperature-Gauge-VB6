...
My current VB6/RC6 PSD program being finished now, 86% complete but on the temperature gauge and anemometer gauges only - with a mostly working ICAO feed source selector and fully functional preference form, an XML parser now generating correct sunrise/sunset times, a textual METAR information sub-form and a completed CHM help file. Awaiting the clock hands to indicate the correct time on the METAR data clock, moving the weather code to its own module, completion of the anemometer gauge, the addition of three of other gauges and then testing on laptops, testing on Windows XP/ReactOS and Win7 32bit, some multi-monitor checking, and the creation of the setup.exe. A bit to do but it work now and is operating on my desktop.
